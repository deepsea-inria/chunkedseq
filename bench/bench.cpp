...
thunk_t scenario_test() {
  ...
  size_t nb_total = (size_t) deepsea::cmdline::parse_or_default_long("n", 100000000);
  size_t repeat = (size_t) deepsea::cmdline::parse_or_default_long("r", 1000);
...
thunk_t scenario_split_merge() {
  typedef typename Datastruct::value_type value_type;
  typedef typename Datastruct::size_type size_type;
  size_t n = (size_t) deepsea::cmdline::parse_or_default_long("n", 100000000);
  size_t p = (size_t) deepsea::cmdline::parse_or_default_long("p", std::max(n/100, (size_type)2));
  size_t r = (size_t) deepsea::cmdline::parse_or_default_long("r", 100000);
  size_t h = (size_t) deepsea::cmdline::parse_or_default_long("h", 0);
  bool should_push = deepsea::cmdline::parse_or_default_bool("should_push", true);
  bool should_pop = deepsea::cmdline::parse_or_default_bool("should_pop", false);
...
thunk_t scenario_filter() {
  typedef typename Datastruct::size_type size_type;
  typedef typename Datastruct::value_type value_type;
  size_t cutoff = deepsea::cmdline::parse_or_default_long("cutoff", 8096);
  size_t n = deepsea::cmdline::parse_or_default_long("n", 100000000);
  size_t r = (size_t) deepsea::cmdline::parse_or_default_long("r", 1);
  size_t nb_total = n / r;
  printf("length %lld\n",(long long int)nb_total);
  const size_t m = 1<<30;
  return [=] {
...
thunk_t scenario_map() {
  ...
  size_t n = deepsea::cmdline::parse_or_default_long("n", 1000000);
  bool test_random = deepsea::cmdline::parse_or_default_bool("test_random", true);
...
void dispatch_for_chunkedseq() {
  ...
  #endif
  c.dispatch_or_default("chunk_size", std::to_string(default_chunksize));
}
...
void dispatch_by_itemsize() {
  ...
  #endif
  c.dispatch_or_default("itemsize", std::to_string(default_itemsize));
}
...
void dispatch_by_benchmark_mode() {
  ...
  c.dispatch_or_default("mode", "sequence");
}
